...
# Hermes / Astral / GUI installers look for ~/.local/bin/uv explicitly. Mise
# keeps the real binary under ~/.local/share/mise/installs; expose a stable
# shim so sanitized PATHs (Ollama CLI, non-interactive bash, Cursor sandbox)
# still find it. Same pattern for bun.
print "Linking mise uv/bun shims into ~/.local/bin..."
if [[ -x $HOME/.local/bin/mise ]]; then
    command $HOME/.local/bin/mise reshim uv >/dev/null 2>&1 || true
    command $HOME/.local/bin/mise reshim bun >/dev/null 2>&1 || true
fi
if [[ -x $XDG_DATA_HOME/mise/shims/uv ]]; then
    zf_ln -sfn $XDG_DATA_HOME/mise/shims/uv $HOME/.local/bin/uv
    print "  ...uv done"
else
    print "  ...uv skipped (mise uv shim missing — run: mise install uv)"
fi
if [[ -x $XDG_DATA_HOME/mise/shims/bun ]]; then
    zf_ln -sfn $XDG_DATA_HOME/mise/shims/bun $HOME/.local/bin/bun
    print "  ...bun done"
else
    print "  ...bun skipped (mise bun shim missing — run: mise install bun)"
fi
